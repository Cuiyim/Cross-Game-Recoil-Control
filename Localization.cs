...
public static class Localization
{
    ...
    {
        ["App.Title"] = new("Legendary v3.1.8", "Legendary v3.1.8"),
        ...
        ["Ui.HelpTitle"] = new("使用说明 v3.1.8", "Usage Guide v3.1.8"),
        ...
        ["Ui.Interval"] = new("空闲轮询(ms)", "Idle poll (ms)"),
        ["Tooltip.IdlePoll"] = new("画面静止时多久重新检查一次（毫秒）。不影响游戏内反应速度——目标出现时会被新帧立即唤醒；仅影响静止时的 CPU 占用，以及 DXGI 不可用时的 GDI 回退轮询。", "How often to re-check while the screen is static (ms). Does NOT affect in-game reaction speed — a new frame wakes detection the instant the target appears. It only changes idle CPU use, plus the GDI fallback poll when DXGI is unavailable."),
        ...
        ["Ui.ConfigFile"] = new("配置文件", "Configuration File"),
        ["Ui.ImportConfig"] = new("导入配置", "Import Config"),
        ["Ui.ExportConfig"] = new("导出配置", "Export Config"),
        ...
        ["Label.Interval"] = new("空闲轮询", "Idle poll"),
        ["Ui.StickyTab"] = new("按键黏连", "Key Sticky"),
        ["Ui.StickyEnable"] = new("启用按键黏连", "Enable key sticky"),
        ["Ui.StickyHint"] = new("点一下触发键，目标键就会一直按住；再点一下松开。适合单手玩家把移动键锁定。", "Tap the trigger key to hold the lock key down; tap again to release. Lets one-handed players lock a movement key."),
        ["Ui.StickyTrigger"] = new("触发键", "Trigger key"),
        ["Ui.StickyTarget"] = new("锁定键", "Lock key"),
        ["Ui.StickyPick"] = new("选择", "Pick"),
        ["Ui.StickyAdd"] = new("添加黏连规则", "Add sticky rule"),
        ["Ui.StickyAdded"] = new("已添加的规则", "Added rules"),
        ["Ui.StickyRemove"] = new("删除选中", "Remove selected"),
        ["Picker.StickyTrigger"] = new("选择触发键", "Pick trigger key"),
        ["Picker.StickyTarget"] = new("选择锁定键", "Pick lock key"),
        ["Sticky.NeedKeys"] = new("请先选择触发键和锁定键", "Pick both a trigger key and a lock key first."),
        ["Sticky.SameKey"] = new("触发键和锁定键需不同：相同键的黏连需要拦截原按键，将在后续步骤支持。", "Trigger and lock key must differ for now; same-key sticky needs key suppression, coming in a later step."),
        ["Sticky.Added"] = new("已添加黏连规则：{0} → 锁定 {1}", "Sticky rule added: {0} → lock {1}"),
        ["Rule.Tab"] = new("自定义规则", "Custom Rules"),
        ["Rule.Enable"] = new("启用自定义规则", "Enable custom rules"),
        ["Rule.Hint"] = new("把按键行为拆成三大类：持续类（长按 / 定速点击）、触发类（一个键触发多个键，可设延迟）、图像识别类（按屏幕颜色触发）。受热键总开关控制。", "Three rule kinds: Continuous (hold / auto-click), Trigger (one key fires several keys, with optional delays), and Image Match (fires on a screen colour). Gated by the hotkey master."),
        ["Rule.New"] = new("新建规则", "New Rule"),
        ["Rule.Category"] = new("规则类型", "Rule Type"),
        ["Rule.CatContinuous"] = new("持续类", "Continuous"),
        ["Rule.CatTrigger"] = new("触发类", "Trigger"),
        ["Rule.CatImage"] = new("图像识别类", "Image Match"),
        ["Rule.TargetKey"] = new("目标键", "Target key"),
        ["Rule.SustainMode"] = new("持续方式", "Sustain Mode"),
        ["Rule.Hold"] = new("长按", "Hold"),
        ["Rule.AutoClick"] = new("定速点击", "Auto Click"),
        ["Rule.EngageMode"] = new("触发模式", "Engage Mode"),
        ["Rule.WhileHeld"] = new("按住触发", "While Held"),
        ["Rule.Toggle"] = new("切换锁定", "Toggle Lock"),
        ["Rule.Rate"] = new("点击/分钟", "Clicks per min"),
        ["Rule.ContinuousHint"] = new("“点击/分钟”仅在“定速点击”时生效；“切换锁定”=点一下锁住、再点松开。", "Clicks per min applies only to Auto Click; Toggle Lock = tap once to lock, tap again to release."),
        ["Rule.FormTap"] = new("点击", "Click"),
        ["Rule.FormDown"] = new("按下", "Press"),
        ["Rule.FormUp"] = new("抬起", "Release"),
        ["Rule.Cooldown"] = new("冷却ms", "Cooldown (ms)"),
        ["Rule.ScanInterval"] = new("扫描间隔ms", "Scan interval (ms)"),
        ["Rule.ImageHint"] = new("图像识别类由屏幕颜色触发，无需触发键；可在游戏进程内工作。", "Image Match fires on a screen colour, needs no trigger key, and works inside the game process."),
        ["Rule.Add"] = new("添加规则", "Add Rule"),
        ["Rule.Added"] = new("已添加规则", "Rule added"),
        ["Rule.NeedAction"] = new("请先添加至少一个动作", "Add at least one action first."),
        ["Rule.NameTrigger"] = new("触发 {0} → {1}个动作", "Trigger {0} → {1} action(s)"),
        ["Rule.NameImage"] = new("图像 → {0}个动作", "Image → {0} action(s)"),
        ["Rule.ActionsHint"] = new("动作（可加多个：同时触发→延迟都填 0；错峰触发→填不同延迟）", "Actions (add several: fire together → all delays 0; stagger → different delays)"),
        ["Rule.ActionTargets"] = new("目标键", "Target key"),
        ["Rule.F2OnStatus"] = new("自定义规则已开启 (F2)", "Custom rules ON (F2)"),
        ["Rule.F2OffStatus"] = new("自定义规则已关闭 (F2)", "Custom rules OFF (F2)"),
        ["Rule.F2On"] = new("F2：自定义规则开启", "F2: custom rules enabled"),
        ["Rule.F2Off"] = new("F2：自定义规则关闭", "F2: custom rules disabled"),
        ...
        ["Config.ImportDialogTitle"] = new("导入配置文件", "Import Configuration File"),
        ["Config.ExportDialogTitle"] = new("导出配置文件", "Export Configuration File"),
        ["Config.FileFilter"] = new("配置文件 (*.json)|*.json|所有文件 (*.*)|*.*", "Configuration files (*.json)|*.json|All files (*.*)|*.*"),
        ["Config.Imported"] = new("配置已导入：{0}", "Configuration imported: {0}"),
        ["Config.Exported"] = new("配置已导出：{0}", "Configuration exported: {0}"),
        ["Config.ImportFailed"] = new("导入失败：配置文件无效或无法读取", "Import failed: invalid or unreadable configuration file"),
        ["Config.ImportError"] = new("导入失败：{0}", "Import failed: {0}"),
        ["Config.ExportError"] = new("导出失败：{0}", "Export failed: {0}"),
